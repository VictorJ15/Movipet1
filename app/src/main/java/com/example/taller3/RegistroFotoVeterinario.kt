...
import com.example.taller3.databinding.RegistroFotoVeterinarioBinding
...
class RegistroFotoVeterinario : AppCompatActivity() {

    private lateinit var binding: RegistroFotoVeterinarioBinding
    private lateinit var file: File

    @RequiresApi(Build.VERSION_CODES.Q)
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = RegistroFotoVeterinarioBinding.inflate(layoutInflater)
    ...
    private fun subirImagenFirebase(uri: Uri, callback: (Boolean) -> Unit) {
        ...
        val storageRef = firebaseStorage.reference.child("Fotos de perfil veterinarios").child("${System.currentTimeMillis()}.jpg")
        ...
        storageRef.putFile(uri).addOnSuccessListener { uploadTask ->
            uploadTask.storage.downloadUrl.addOnSuccessListener { downloadUri ->
                ...
                firebaseDatabase.reference.child("veterinarios").child(firebaseAuth.currentUser!!.uid).updateChildren(updateData)
